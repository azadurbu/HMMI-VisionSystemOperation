...
namespace VisionSystemOperation.Device.PLC_Library
{
    ...
    class XGCommSocket
    {
        ...
        private bool IsConnSucees { get; set; } = false;
        //public bool IsConnSucees {
        //    get {
        //        if (this.m_CommDriver == null) return false;
        //        if (this.m_CommDriver.IsConnected() != (uint)XGCOMM_FUNC_RESULT.RT_XGCOMM_SUCCESS)
        //            return false;
        //        else
        //            return true;
        //    }
        ...
        public uint UpdateKeepAlive()
        {
            ...
            if (!IsConnSucees) return (uint)XGCOMM_FUNC_RESULT.RT_XGCOMM_FAILED_CONNECT;
        ...
        public uint ReadDataWord(char szDeviceType, long lOffsetWord, long lSizeWord, bool bByteSwap, UInt16[] pwRead, ref System.Collections.BitArray outBitArray)
        {
            if (this.m_CommDriver == null)
            {
                return (uint)XGCOMM_FUNC_RESULT.RT_XGCOMM_INVALID_COMM_DRIVER;
            ...
            outBitArray = new System.Collections.BitArray(bufRead);
            bool[] bitarray = new bool[outBitArray.Length];
            outBitArray.CopyTo(bitarray, 0);
            //string bitStr = "";
            //for (int i = 0; i < bitArray.Length; i++)
            //{
            //    bitarray[i] = bitArray[i];
            //    bitStr += bitarray[i].ToString();
            //}


            string val = Encoding.Default.GetString((bufRead), 0, (int)lSizeByte);
            //char[] bitData = Convert.ToString(bufRead, 2).PadLeft(16, '0').ToArray(); /// Seq Status 확보 // ex) 2452 -> 0000 1001 1001 0100 // Hex2Bit
            ...
            if (dwReturn == (uint)XGCOMM_FUNC_RESULT.RT_XGCOMM_SUCCESS)
            {
                if (pwRead != null)
                {
                    ...
                    //    {
                    //        if(bitArray[i])
                    //            pwRead[lCount] |= (UInt16)(1 << i);
                    //    }
                    //}

                    //UInt16 MSB = bufRead[0];
                    //UInt16 LSB = bufRead[1];

                    //pwRead[0] = (UInt16)(((MSB << 8) & 0xFF00) + LSB);
                    if (bByteSwap == true)
                    {
                        for (lCount = 0; lCount < lSizeWord; lCount++)
                        {
                            lByteOffset = lCount * 2;
                            pwRead[lCount] = MAKEWORD((bufRead[lByteOffset]), (bufRead[lByteOffset + 1]));
                        }
                    }
                    else
                    {
